...
```{r}
box_sm = data.q.change.long
# --------------------------------------------------------
gc()
remove(simulation2_results)
graphics.off()
cat("\014")
# --------------------------------------------------------
```
...
```{r}
box_random = data.q.change.long
```

# Compare networks together

```{r}
# box_full = box_random + box_sm
box_full = rbind(box_random, box_sm)
```
...
box_combine = ggplot(box_full) +
  aes(x = q_number, y = Steps_Count, fill = network_type) +
...
ggsave("./pic_q_change_research/box_combine.png",
       plot = box_combine,
       device = "png",
       height = 8,
       width = 15)

ggsave("./pic_q_change_research/box_combine.svg",
       plot = box_combine,
       device = "svg",
       height = 8,
       width = 15)
