...
```{r}
library(data.table)
simulation3_results = fread("./report/simulation3.csv")
```
...
ggsave("./pic_same_inno_prop_research/box_sm.png",
       plot = box,
       device = "png",
       height = 9,
       width = 14)

ggsave("./pic_same_inno_prop_research/box_sm.svg",
       plot = box,
       device = "svg",
       height = 9,
       width = 14)
...
```{r}
simulation9_results = fread("./report/simulation9.csv")
data.box = simulation9_results %>%
  filter(AgentID == 0, Step == 100)
data.box$agent_proportion_list = lapply(data.box$agent_proportion, fromJSON)
data.box$prop_inf_inno = sapply(data.box$agent_proportion_list, function(x) x[1])
...
ggsave("./pic_same_inno_prop_research/box_random.png",
       plot = box,
       device = "png",
       height = 9,
       width = 14)

ggsave("./pic_same_inno_prop_research/box_random.svg",
       plot = box,
       device = "svg",
       height = 9,
       width = 14)
...
box_combine = ggplot(box) +
  aes(x = prop_inf_inno_number, y = Steps_Count, fill = network_type) +
  ...
  scale_fill_manual(values = c("small_world" = "#6888F5", "random" = "#D77071"),
                    name = "Network Type") +
  labs(
    x = "Different Proportion of Influncial Innovator",
    y = "Steps Count",
    title = "Research on the Influncial Innnovator",
    subtitle = "When the proportion of Innovators fixed to 0.01"
  ) +
  theme_light() +
  theme(
    plot.title = element_text(face = "bold", hjust = 0.5),
...
ggsave("./pic_same_inno_prop_research/box_combine.png",
       plot = box_combine,
       device = "png",
       height = 8,
       width = 15)

ggsave("./pic_same_inno_prop_research/box_combine.svg",
       plot = box_combine,
       device = "svg",
       height = 8,
       width = 15)
