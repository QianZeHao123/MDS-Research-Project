...
library(data.table)
model5_results = fread("./model5results.csv")
```
...
```{r message=FALSE, warning=FALSE}
library(dplyr)
network_research_id0 = model5_results %>%
  filter(RunId == 0)

remove(model5_results)
```
...
agent_info_id0 = network_research_id0 %>%
  filter(Step == 0) %>%
  select(AgentID, Agent_Type, Neighbors, Influencer)
```
...
for (step in steps) {
  ...
  ggsave(
    filename = paste0("./pic/network_plot_step_", step, ".png"),
    plot = p,
    width = 10,
    height = 10
  )
